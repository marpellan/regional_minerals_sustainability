...
def plot_iwplus_contributions(df, save_path_eco=None, save_path_hh=None):
    ...
    fig_eco.legend(handles_eco, labels_eco, loc='upper center', ncol=4, bbox_to_anchor=(0.5, -0.1), fontsize=12)
    plt.tight_layout()
    if save_path_eco:
    ...
    # Legend with increased font size and bottom placement
    fig_hh.legend(handles_hh, labels_hh, loc='upper center', ncol=4, bbox_to_anchor=(0.5, -0.1), fontsize=12)
    plt.tight_layout()
    if save_path_hh:
...
def heatmap_lca(df, colors=None, title=None, save_path=None):
    ...
    ax.set_xticklabels([f"{col}" for col in df.columns], rotation=90, ha='right', fontsize=14,
                       fontweight='bold')
...
def heatmap_db_comparison(df1, df2, title=None, save_path=None):
    ...
    Parameters:
        df1 (pd.DataFrame): DataFrame with 'Commodity' as index and impact categories as columns.
        df2 (pd.DataFrame): DataFrame with 'Commodity' as index and impact categories as columns.
    ...
    # Use diverging color map with midpoint at zero
    cmap = plt.get_cmap("coolwarm")
    norm = TwoSlopeNorm(vmin=df_diff.values.min(), vcenter=0, vmax=df_diff.values.max())

    # Create grid of cells for heatmap
    ...
    ax.set_xticks(np.arange(num_cols) + 0.5)
    ax.set_yticks(np.arange(num_rows) + 0.5)
    ax.set_xticklabels(df_diff.columns, rotation=90, ha='right', fontsize=10, fontweight='bold')
    ax.set_yticklabels(df_diff.index, fontsize=10, fontweight='bold')

    # Add horizontal lines to separate commodities
    ax.hlines(np.arange(1, num_rows), xmin=0, xmax=num_cols, color="black", linewidth=0.5)
    ax.set_xlim(0, num_cols)
    ax.set_ylim(num_rows, 0)

    # Set title with customized size, boldness, and spacing
    if title:
        plt.title(title, fontsize=18, fontweight='bold', pad=30)

    # Ensure everything fits within the saved image
    plt.tight_layout(rect=[0, 0, 1, 0.95])  # Leaves space at the top for title
    if save_path:
...
def plot_production_impacts(projected_df, production_df,
    ...
    labels = minerals
    fig.legend(handles, labels, loc="lower center", bbox_to_anchor=(0.5, -0.05), ncol=len(minerals), fontsize=18)
